...
float vec3_norm_dot_product(vec_3_t p1, vec_3_t p2){
    p1 = vec3_scalar_mult(p1, 1/vec3_magnitude(p1));
    p2 = vec3_scalar_mult(p2, 1/vec3_magnitude(p2));
    return vec3_dot_product(p1, p2);
}
...
vec_3_t get_tris_normal(tris_t tris){
    ...
    return vec3_cross_product(p0p1, p0p2);
}

vec_3_t vec3_normalize(vec_3_t in){
    return vec3_scalar_mult(in, 1/vec3_magnitude(in));
}
...
vec_3_t vec3_generate_random_vector(){
    vec_3_t retval;
    retval.x = ((rand()/(RAND_MAX / 2.0)) - 1.0);
    retval.y = ((rand()/(RAND_MAX / 2.0)) - 1.0);
    retval.z = ((rand()/(RAND_MAX / 2.0)) - 1.0);
    return(retval);
}